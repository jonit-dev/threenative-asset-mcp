 ...
 * missing measurement is honest where a wrong one is not. That covers a file needing a decoder
 * this server does not carry (Draco, meshopt) and a scene with no mesh to bound, whose box
 * `getBounds` reports as the non-finite -Infinity rather than as a size.
 */
export async function sizeMeters(path: string): Promise<SizeMeters | undefined> {
  ...
  try {
    const root = (await new NodeIO().registerExtensions(ALL_EXTENSIONS).read(path)).getRoot();
    const scene = root.getDefaultScene() ?? root.listScenes()[0];
    if (!scene) return undefined;
    const { min, max } = getBounds(scene);
    const size = { x: max[0] - min[0], y: max[1] - min[1], z: max[2] - min[2] };
    return [size.x, size.y, size.z].every(Number.isFinite) ? size : undefined;
  } catch {
    return undefined;
  }
}
